...
public class TurnResource {
    ...
    private final TurnSearchRepository turnSearchRepository;

    public TurnResource(TurnRepository turnRepository, TurnSearchRepository turnSearchRepository) {
        this.turnRepository = turnRepository;
        this.turnSearchRepository = turnSearchRepository;
    }
    ...
    public ResponseEntity<Turn> createTurn(@Valid @RequestBody Turn turn) throws URISyntaxException {
        ...
        turnSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteTurn(@PathVariable Long id) {
        ...
        turnSearchRepository.deleteById(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
    }

    /**
     * SEARCH  /_search/turns?query=:query : search for the turn corresponding
     * to the query.
     *
     * @param query the query of the turn search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/turns")
    @Timed
    public ResponseEntity<List<Turn>> searchTurns(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Turns for query {}", query);
        Page<Turn> page = turnSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/turns");
