...
try {
  ...
  stage ('Analysis') {
    ...
    def sonarParams = "-Dsonar.host.url=https://sonarcloud.io -Dsonar.projectKey=bmordue_lgm-java-no-client -Dsonar.organization=bmordue-github -Dsonar.java.binaries=./target/ -Dsonar.coverage.jacoco.xmlReportPaths=target/test-results/coverage/jacoco/jacoco.xml -Dsonar.tests=src/test"
